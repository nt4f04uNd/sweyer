...
class PlayerManager {
  ...
  static PlayerManager get instance {
    return _instance ??= PlayerManager._();
  }

  static PlayerManager? get instanceIfInitialized {
    return _instance;
  }
  ...
  Future<void> init() async {
    ...
    // Restore from prefs.
    await Future.wait([
      _player.seek(Duration(seconds: Prefs.songPosition.get())),
      _player.setLoopMode(Prefs.loopMode.get() ? LoopMode.one : LoopMode.off),
    ]);
  }
...
class AudioHandler extends BaseAudioHandler with SeekHandler, WidgetsBindingObserver {
  AudioHandler(PlayerManager player) {
    _init(player);
  }
